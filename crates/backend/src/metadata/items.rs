...
impl<'a> MetadataItem for TechnicSearchMetadataItem<'a> {
    ...
    fn request(&self, client: &reqwest::Client) -> RequestBuilder {
        client.get(TECHNIC_SEARCH_URL)
            .query(self.0)
            .header("User-Agent", "PandoraLauncher/1.0")
    }
